...
### Epic 2: 历史导航与日期浏览
用户可以通过日历弹窗和日期路由浏览任意历史日期的资讯，支持日历选择日期、前后日期切换，复用首页布局展示历史内容。
...
**设计稿参考：** XqEim 头部区域, OSbpZ 头部区域, 组件 1tPAu (DateNav), lBzjR (SearchBox)
...
**设计稿参考：** XqEim (Homepage-Desktop), OSbpZ (Mobile-Homepage), 组件 hwpF3 (NewsCard), O8R2L (DateGroupHeader)
...
**设计稿参考：** XqEim (桌面端 1440px), OSbpZ (移动端 390px)
...
**目标：** 用户可以通过日历弹窗和日期路由浏览任意历史日期的资讯内容，支持日历选择日期、前后日期切换。历史日期复用首页布局，无独立归档页面。
...
**设计稿参考：** rhhn2 (calendarPopup 桌面端日历浮层, 276px 宽), 1W03N (CalendarBottomSheet 移动端底部弹出面板，含遮罩层)

**验收标准：**

**Given** 用户正在浏览首页或任意日期的资讯页面
**When** 用户点击 Header 日期区域的日历触发按钮
**Then** 桌面端弹出浮层日历（对应设计稿 rhhn2），宽度 276px，定位于触发按钮下方
**And** 移动端弹出底部面板日历（对应设计稿 1W03N），从屏幕底部滑入
**And** 移动端日历弹出时显示半透明遮罩层，点击遮罩关闭日历
**And** 日历中有数据的日期可点击选择，无数据的日期显示为禁用状态
**And** 当前正在浏览的日期在日历中高亮标记

**When** 用户在日历中选择某个日期
**Then** 日历弹窗自动关闭
**And** 首页内容切换为所选日期的资讯（复用首页布局，无独立归档页面）
**And** URL 更新为对应日期路由（如 /daily/2026-02-25/）

**And** 日历组件支持键盘导航（方向键切换日期，Enter 选择，Escape 关闭）（NFR15）
**And** 日历触发按钮包含 aria-haspopup 和 aria-expanded 属性
**And** 遮罩层和弹窗使用 role="dialog" 和 aria-modal="true"
...
**设计稿参考：** P3UuJ (Search-Desktop), 59Oal (Search-Mobile)
...
| FR9 | 日历弹窗选择历史日期 | Epic 2 | Story 2.3 | ✅ 完整覆盖 |
