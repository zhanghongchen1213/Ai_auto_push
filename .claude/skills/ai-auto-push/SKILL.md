...
**CRITICAL - 必须使用标准模板文件生成内容：**

1. 使用 Read 工具读取标准模板：`{PROJECT_ROOT}/.claude/skills/ai-auto-push/templates/standard.md`
2. 将模板中的占位符替换为实际内容
3. 使用 Write 工具写入文件：`{PROJECT_ROOT}/src/content/daily/{date}/{slug}.md`

**占位符说明：**
- `{{DOMAIN_NAME}}` - 领域名称（如"AI技术"、"跨境电商"）
- `{{DOMAIN_SLUG}}` - 领域slug（如"ai-tech"、"cross-border-ecom"）
- `{{DATE}}` - 日期（YYYY-MM-DD格式）
- `{{ITEM_COUNT}}` - 条目数量
- `{{GENERATED_AT}}` - 生成时间（ISO8601格式，UTC+8）
- `{{TITLE_N}}` - 第N条资讯标题
- `{{SUMMARY_N}}` - 第N条资讯摘要（150-200字）
- `{{SOURCE_NAME_N}}` - 第N条资讯来源名称
- `{{SOURCE_URL_N}}` - 第N条资讯来源URL

**模板重复规则：**
- 模板中只有2条示例，实际生成时需要重复"标题+摘要+来源"块
- ai-tech 领域最多20条，其他领域最多10条
- 每条之间空一行

**严格遵循以下格式模板（CRITICAL - 每个字段都是必需的）：**
...
**来源：** [{来源名称}]({来源URL}) | **原文：** [{原文标题}]({原文URL})
```

**CRITICAL - 来源链接是强制要求：**
- 每条资讯**必须**有 `**来源：** [名称](URL)` 格式的链接
- 必须使用中文冒号 `：` 而非英文冒号 `:`
- 必须是 Markdown 链接格式 `[文本](URL)`，不能是纯文本
- URL 必须是真实可访问的链接，不能是占位符
- 如果有聚合站点和原文，使用双链接格式：`**来源：** [聚合站](URL) | **原文：** [原文](URL)`
...
**格式规则（CRITICAL - 必须严格遵守）：**

✅ **允许的格式：**
- frontmatter 后直接开始第一条资讯（`## 标题`）
- 每条资讯使用 `## 标题` 格式（二级标题，无编号）
- 每条资讯包含摘要段落和来源链接

❌ **严格禁止的格式（违反将导致构建失败）：**
- ❌ 禁止使用顶级标题 `#`（如 `# GitHub Trending 2026-03-07`）
- ❌ 禁止使用编号标题（如 `## 1. 标题` 或 `### 1. 标题`）
- ❌ 禁止使用分类章节标题（如 `## 核心技术突破`、`## 今日概况`、`## 开源项目动态`）
- ❌ 禁止在 frontmatter 后添加概述段落（如"今日共检索 XX 条..."）
- ❌ 禁止使用三级或更深标题 `###`（除非在 ai-tech 领域用于编号项）
- ❌ **禁止缺少来源链接** - 每条资讯必须有 `**来源：** [名称](URL)` 格式的链接
- ❌ **禁止使用纯文本来源** - 必须是 Markdown 链接格式，不能是 `**来源**: 纯文本`
- ❌ **禁止使用英文冒号** - 必须使用中文冒号 `：` 而非 `:`
...
- 来源名称中的 `[` `]` 需转义：`\[` `\]`
- 文件末尾保留一个换行符
- **来源与原文双链接格式**：当资讯来自聚合站点时，使用 `**来源：** [名称](聚合页URL) | **原文：** [标题](原文URL)` 格式
- 如果资讯直接来自原始来源（非聚合转载），只需使用单链接格式 `**来源：** [名称](原文URL)`

**生成后自检清单：**
在调用 Write 工具前，必须确认生成的内容：
- [ ] frontmatter 后没有概述段落，直接是第一条 `## 标题`
- [ ] 所有标题都是 `## 标题` 格式（无 `#` 无编号）
- [ ] 没有任何分类章节标题
- [ ] 每条资讯都有 `**来源：**` 链接

**即时格式检查（Write 后立即执行 - CRITICAL）：**

每个文件生成后，**必须**立即运行严格格式检查：

```bash
bash {PROJECT_ROOT}/scripts/check-format-strict.sh {PROJECT_ROOT}/src/content/daily/{date}/{slug}.md
```

严格检查验证：
- ✅ frontmatter 后直接是 `## 标题`（无概述段落）
- ✅ 每条资讯都有 `**来源：** [名称](URL)` 格式的链接
- ✅ 来源必须是链接格式，不能是纯文本
- ✅ 使用中文冒号 `：` 而非英文冒号 `:`
...
2. 使用 Edit 工具修复文件：
   - 删除概述段落
   - 为每条资讯添加 `**来源：** [名称](URL)` 链接
   - 修正来源格式（必须是链接，不能是纯文本）
3. 重新运行检查直到通过

**禁止跳过此检查步骤。格式错误的文件即使构建成功也无法正确显示。**
